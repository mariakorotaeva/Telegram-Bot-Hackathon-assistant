...
def get_visibility_keyboard():
    ...
    builder.button(text="❌ Отмена", callback_data="visibility_cancel")
...
async def process_event_description(message: Message, state: FSMContext):
    description = message.text if message.text.lower() != "-" else ""
...
async def process_event_location(message: Message, state: FSMContext):
    location = message.text if message.text.lower() != "-" else ""
...
async def process_visibility(callback: CallbackQuery, state: FSMContext, bot: Bot):
...
async def admin_edit_event_list(callback: CallbackQuery):
    ...
    for event in events:
        ...
        builder.button(text=button_text, callback_data=f"edit_event:{event['id']}")
...
@router.callback_query(F.data.startswith("edit_") & F.data.contains(":"))
async def admin_edit_field(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await state.update_data(
            edit_action=action.replace("edit_", ""),
            edit_event_id=event_id
        )
        if action == "edit_title":
            await callback.message.edit_text("✏️ Введите новое название события:", parse_mode="HTML")
            ...
            current_duration = int((event["end_time"] - event["start_time"]).total_seconds() / 60)
            await callback.message.edit_text(
                f"⏱️ Введите новую продолжительность в минутах:",
                parse_mode="HTML"
            )
        ...
        print(f"Error parsing callback data {callback.data}: {e}")
    except Exception as e:
        await callback.answer("❌ Произошла ошибка", show_alert=True)
        print(f"Unexpected error in admin_edit_field: {e}")
...
async def process_edit_value(message: Message, state: FSMContext, bot: Bot):
    ...
    if not action or not event_id:
        await message.answer("❌ Ошибка: данные не найдены", show_alert=True)
        await state.clear()
        return
    try:
        value = message.text.strip()
        if action in ["title", "description", "location"]:
            if value == "-":
                value = ""
            if action == "title" and not value:
            ...
            new_start_time = datetime.strptime(value, "%d.%m.%Y %H:%M")
            now = datetime.now()
            ...
            await message.answer("❌ Неизвестное действие", show_alert=True)
            await state.clear()
            return
        if update_success:
            ...
            await message.answer(
                f"❌ Не удалось обновить {action_display.lower()}!",
                reply_markup=get_edit_event_keyboard(event_id),
                parse_mode="HTML"
            )
        await state.clear()
        ...
        error_msg = {
            "start": "❌ Неверный формат даты! Используйте ДД.ММ.ГГГГ ЧЧ:ММ\nНапример: 15.12.2025 10:00",
            "duration": "❌ Неверный формат! Введите число минут (например: 60, 90, 120)"
        }.get(action, "❌ Ошибка в данных")
        await message.answer(error_msg, show_alert=True)
    except Exception as e:
        await message.answer("❌ Произошла ошибка", show_alert=True)
        print(f"Error in process_edit_value: {e}")
        await state.clear()
