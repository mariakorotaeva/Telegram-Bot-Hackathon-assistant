...
class FormCreationState(StatesGroup):
...
async def view_profiles(callback: CallbackQuery):
    ...
    if not profiles:
        ...
        if 'К сожалению, сейчас нет активных анкет других участников' not in callback.message.text:
            await callback.message.edit_text(
                "👀 <b>Анкеты участников</b>\n\n"
    ...
    for i, profile_user in enumerate(profiles, 1):
        ...
        profile_info_raw = (
            f"{i}. {profile_user.full_name}\n"
            f"📱 Telegram: {tg_username}\n"
            f"📝 {preview}\n"
            f"─────────────────"
        )
        profiles_raw.append(profile_info_raw)
    ...
    full_text_raw = (
        "👀 Активные анкеты участников\n\n"
        "Вот случайные анкеты других участников, которые ищут команду:\n\n"
    ) + "\n\n".join(profiles_raw)
    
    if full_text_raw != callback.message.text:
        builder = InlineKeyboardBuilder()
...
async def my_profile(callback: CallbackQuery):
    ...
    if is_empty:
        ...
        if user.profile_active and not has_team:
...
async def toggle_profile_active(callback: CallbackQuery):
    ...
    new_active_status = not user.profile_active
...
async def edit_profile_start(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(FormCreationState.waiting_for_text)
...
@router.message(FormCreationState.waiting_for_text)
